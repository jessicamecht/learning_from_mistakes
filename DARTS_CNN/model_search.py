...
class Network(nn.Module):
  ...
  def _initialize_alphas(self):
    ...
    self.alphas_normal = Variable(1e-3*torch.randn(k, num_ops), requires_grad=True)#.cuda(), requires_grad=True)
    self.alphas_reduce = Variable(1e-3*torch.randn(k, num_ops), requires_grad=True)#.cuda(), requires_grad=True)
